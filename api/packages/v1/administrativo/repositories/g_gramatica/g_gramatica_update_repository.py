...
class GGramaticaUpdateRepository(BaseRepository):
    """
    ...
    def execute(self, g_gramatica_update_schema: GGramaticaUpdateSchema):
        """
        Atualiza um registro existente na tabela G_GRAMATICA.
        ...
        try:
            ...
            params, update_columns = prepare_update_data(
                g_gramatica_update_schema,
                exclude_fields=["gramatica_id"],
            ...
            sql = f"""
                UPDATE G_GRAMATICA
                SET {update_columns}
                WHERE GRAMATICA_ID = :gramatica_id
                RETURNING *;
            """
            ...
            raise HTTPException(
                ...
                detail=f"Erro ao atualizar registro em G_GRAMATICA: {str(e)}",
            )
