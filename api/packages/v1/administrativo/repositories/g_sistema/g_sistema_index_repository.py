...
_GSISTEMA_SORT_FIELD_MAP = {
    "sistema_id": "SISTEMA_ID",
    "descricao": "DESCRICAO",
    "situacao": "SITUACAO",
    "tipo_cartorio": "TIPO_CARTORIO",
    "versao": "VERSAO",
...
class IndexRepository(BaseRepository):
    def execute(
        ...
        sort_field, sort_direction = QueryParamsParser.resolve_sort(
            query_params,
            primary_key="sistema_id",
            field_map=_GSISTEMA_SORT_FIELD_MAP,
        )

        if use_orm_firebird() and (
            firebird_orm_supports_string_where()
            or not self._has_string_filters(sistema_index_schema)
        ):
            return self._execute_orm(
                sistema_index_schema, page, per_page, sort_field, sort_direction
            )
    ...
    def _execute_orm(
        ...
        where = self._build_orm_where(sistema_index_schema)
        ...
        result = get_g_sistema_model().findAndCountAll(options)
        total = int(result.get("count") or 0)
        rows = result.get("rows") or []

        return {
            "rows": [self._map_sistema_row(row) or {} for row in rows],
    ...
    def _execute_sql(
        ...
        where_clauses, params = self._build_sql_filters(sistema_index_schema)
        where_sql = self._where_sql(where_clauses)
        offset = (page - 1) * per_page

        count_sql = f"SELECT COUNT(*) AS TOTAL FROM G_SISTEMA{where_sql}"
    ...
    @staticmethod
    def _has_string_filters(sistema_index_schema: GSistemaIndexSchema) -> bool:
        return any(
            [
                sistema_index_schema.situacao is not None,
                sistema_index_schema.descricao is not None,
                sistema_index_schema.tipo_cartorio is not None,
            ]
        )

    @staticmethod
    def _build_orm_where(sistema_index_schema: GSistemaIndexSchema) -> dict[str, Any]:
        clauses: list[dict[str, Any]] = []

        if sistema_index_schema.tipo_cartorio is not None:
            clauses.append({"TIPO_CARTORIO": sistema_index_schema.tipo_cartorio})
        if sistema_index_schema.situacao == SITUACAO_CODIGO_ATIVO:
            clauses.append({"SITUACAO": SITUACAO_CODIGO_ATIVO})
        elif sistema_index_schema.situacao == SITUACAO_CODIGO_INATIVO:
            clauses.append(
                {
                    Op.or_: [
        ...
        if sistema_index_schema.descricao is not None:
            clauses.append(
                {"DESCRICAO": {Op.like: f"%{sistema_index_schema.descricao}%"}}
            )
    ...
    def _build_sql_filters(
        self, sistema_index_schema: GSistemaIndexSchema
    ) -> tuple[list[str], dict[str, Any]]:
        where: list[str] = []
        params = sistema_index_schema.model_dump(exclude_none=True)

        if sistema_index_schema.tipo_cartorio is not None:
            where.append("UPPER(TRIM(TIPO_CARTORIO)) = UPPER(TRIM(:tipo_cartorio))")
        if sistema_index_schema.situacao is not None:
            self._append_situacao_filter(where, params, sistema_index_schema.situacao)
        if sistema_index_schema.descricao is not None:
            where.append("UPPER(DESCRICAO) LIKE UPPER(:descricao)")
            params["descricao"] = f"%{sistema_index_schema.descricao}%"
