from packages.v1.administrativo.actions.t_censec_naturezatipo.t_censec_tiponatureza_delete_action import (
    TCensecTipoNaturezaDeleteAction,
)
...
class TCensecTipoNaturezaDeleteService:
    
    def execute(self, t_censec_tiponatureza_id_schema: TCensecTipoNaturezaIdSchema):
        
        # Instanciamento da ação
        t_censec_tiponatureza_delete_action = TCensecTipoNaturezaDeleteAction()

        # Execução da ação
        data = t_censec_tiponatureza_delete_action.execute(
        ...
        if data:
            ...
            sequencia_schema = GSequenciaDeleteSchema(
                sequencia=t_censec_tiponatureza_id_schema.censec_tiponatureza_id,
                tabela='T_CENSEC_TIPONATUREZA'
            )
