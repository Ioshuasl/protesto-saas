...
from packages.v1.administrativo.schemas.t_minuta_schema import TMinutaSaveSchema, TMinutaDescricaoSchema
from packages.v1.administrativo.actions.t_minuta.t_minuta_save_action import SaveAction
...
class SaveService:
    ...
    def execute(self, minuta_schema: TMinutaSaveSchema):
        ...
        descricao_service = self.dynamic_import.service("t_minuta_get_descricao_service", "GetByDescricaoService")

        # Instanciamento da service
        self.descricao_service = descricao_service()

        # Verifica se a descrição já está sendo utilizada
        self.response = self.descricao_service.execute(TMinutaDescricaoSchema(descricao=minuta_schema.descricao), False)
        ...
        if not minuta_schema.minuta_id:
            ...
            minuta_schema.minuta_id = sequencia.sequencia

        # Instanciamento de ações
        save_action = SaveAction()

        # Retorna o resultado da operação
        return save_action.execute(minuta_schema)