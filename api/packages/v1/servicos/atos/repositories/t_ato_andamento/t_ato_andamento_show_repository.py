...
class TAtoAndamentoShowRepository(BaseRepository):
    """
    ...
    def execute(self, t_ato_andamento_id_schema: TAtoAndamentoIdSchema):
        ...
        try:
            ...
            sql = """
                SELECT *
                FROM T_ATO_ANDAMENTO GG
                WHERE GG.ATO_ANDAMENTO_ID = :ato_andamento_id
            """

            # ----------------------------------------------------
            # Preenchimento de parâmetros
            # ----------------------------------------------------
            params = t_ato_andamento_id_schema.model_dump(exclude_unset=True)
            ...
            if not result:
                raise HTTPException(
                    ...
                    detail="Registro de T_ATO_ANDAMENTO não encontrado.",
                )
            ...
            raise HTTPException(
                ...
                detail=f"Erro ao buscar registro em T_ATO_ANDAMENTO: {e}",
            )
