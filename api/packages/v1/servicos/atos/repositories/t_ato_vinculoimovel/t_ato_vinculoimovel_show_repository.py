...
class TAtoVinculoImovelShowRepository(BaseRepository):
    """
    ...
    def execute(self, t_ato_vinculoimovel_id_schema: TAtoVinculoImovelIdSchema):
        ...
        try:
            ...
            sql = """
                SELECT *
                FROM T_ATO_VINCULOIMOVEL GG
                WHERE GG.ATO_VINCULOIMOVEL_ID = :ato_vinculoimovel_id
            """

            # ----------------------------------------------------
            # Preenchimento de parâmetros
            # ----------------------------------------------------
            params = t_ato_vinculoimovel_id_schema.model_dump(exclude_unset=True)
            ...
            if not result:
                raise HTTPException(
                    ...
                    detail="Registro de T_ATO_VINCULOIMOVEL não encontrado.",
                )
            ...
            raise HTTPException(
                ...
                detail=f"Erro ao buscar registro em T_ATO_VINCULOIMOVEL: {e}",
            )
