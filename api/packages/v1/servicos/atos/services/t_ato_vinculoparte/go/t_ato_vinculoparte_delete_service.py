...
from packages.v1.servicos.atos.actions.t_ato_vinculoparte.t_ato_vinculoparte_delete_action import (
    TAtoVinculoParteDeleteAction,
)
...
class TAtoVinculoParteDeleteService:
    """
    Serviço responsável por encapsular a lógica de negócio para a operação
    de exclusão de um registro na tabela T_ATO_VINCULOPARTE_VINCULOPARTE.
    """
    ...
    def execute(
        ...
        # ----------------------------------------------------
        t_ato_vinculoparte_delete_action = TAtoVinculoParteDeleteAction()

        # ----------------------------------------------------
        # Valida existência (e levanta 404 se não existir)
        # ----------------------------------------------------
        show_action = TAtoVinculoParteShowAction()
        show_action.execute(t_ato_vinculoparte_id_schema)

        # ----------------------------------------------------
        # Execução da ação
        # ----------------------------------------------------
        data = t_ato_vinculoparte_delete_action.execute(t_ato_vinculoparte_id_schema)
        ...
        sequencia_schema = GSequenciaDeleteSchema(
            sequencia=int(t_ato_vinculoparte_id_schema.ato_vinculoparte_id),
            tabela="T_ATO_VINCULOPARTE",
        )
        seq_service.execute(sequencia_schema)

        # ----------------------------------------------------
        # Grava histórico de exclusão do vínculo de parte
        # ----------------------------------------------------
        now = datetime.now()
        now_br = now.strftime("%d/%m/%Y %H:%M:%S")
        usuario_id = t_ato_vinculoparte_id_schema.usuario_id
        ato_vinculoparte_id = t_ato_vinculoparte_id_schema.ato_vinculoparte_id

        historico_schema = THistoricoSaveSchema(
            tabela="T_ATO_VINCULOPARTE",
            campo="Exclusão de vínculo de parte",
            operacao="E",
            new_value=None,
            usuario_id=usuario_id,
            data=now,
            id=ato_vinculoparte_id,
            observacao=f"Vínculo de parte removido pelo usuário({usuario_id}), no dia {now_br}",
            data_registro=None,
