...
class _FakeController:
    def update_ato_anterior(self, data):
        return {
            "message": "Dados do ato anterior atualizados com sucesso.",
            "data": {
                "ato_id": int(data.ato_id),
                "ato_anterior_origem": data.ato_anterior_origem,
            },
        }
...
def test_t_ato_anterior_update_put_deve_retornar_sucesso(monkeypatch, fake_current_user):
    monkeypatch.setattr(t_ato_endpoint, "controller", _FakeController)
    client = _build_client(fake_current_user)

    response = client.put(
        "/servicos/atos/t_ato/123/ato-anterior",
        json={"ato_anterior_origem": "S"},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["message"] == "Dados do ato anterior atualizados com sucesso."
    assert payload["data"]["ato_id"] == 123
    assert payload["data"]["ato_anterior_origem"] == "S"


def test_t_ato_anterior_update_deve_retornar_405_para_metodo_invalido(fake_current_user):
    client = _build_client(fake_current_user)

    response = client.get("/servicos/atos/t_ato/123/ato-anterior")
