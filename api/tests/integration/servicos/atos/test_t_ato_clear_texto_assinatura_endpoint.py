...
class _FakeController:
    def clear_assinatura(self, data):
        return {
            "message": "Texto de assinatura de T_ATO limpo com sucesso.",
            "data": {
                "ato_id": int(data.ato_id),
                "tipo_assinatura": int(data.tipo_assinatura),
                "texto_assinatura": None,
            },
        }
...
def test_t_ato_clear_texto_assinatura_put_deve_retornar_sucesso(
    ...
    response = client.put("/servicos/atos/t_ato/123/texto/assinatura/1/limpar")

    assert response.status_code == 200
    payload = response.json()
    assert payload["message"] == "Texto de assinatura de T_ATO limpo com sucesso."
    assert payload["data"]["ato_id"] == 123
    assert payload["data"]["tipo_assinatura"] == 1
    assert payload["data"]["texto_assinatura"] is None


def test_t_ato_clear_texto_assinatura_deve_retornar_405_para_metodo_invalido(
    fake_current_user,
):
    client = _build_client(fake_current_user)

    response = client.get("/servicos/atos/t_ato/123/texto/assinatura/1/limpar")
