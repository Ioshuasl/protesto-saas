...
export async function PTituloCancelamentoData(
  id: number,
  ...
  console.log('[PTituloCancelamentoData] fluxo hooks -> service -> data executado (mock)');
  ...
  const payloadDate = parseLocalIsoDateOnly(payload.data_cancelamento);
  const next: PTituloInterface = {
    ...current,
    data_cancelamento: payloadDate ?? current.data_cancelamento,
