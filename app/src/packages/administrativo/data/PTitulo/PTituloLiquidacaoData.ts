...
export async function PTituloLiquidacaoData(
  id: number,
  payload?: PTituloLiquidacaoFormInterface,
  ...
  if (!isPTituloWorkflowMockEnabled()) {
    ...
    const apiCall = withClientErrorHandler(async () =>
      api.send({
        method: Methods.PUT,
        endpoint: PTITULO_WORKFLOW_ENDPOINTS.liquidarTitulo(id),
  ...
  console.log('[PTituloLiquidacaoData] fluxo hooks -> service -> data executado (mock)');
  ...
  const payloadDate = parseLocalIsoDateOnly(payload.data_liquidacao);
  const next: PTituloInterface = {
    ...current,
    data_pago: payloadDate ?? current.data_pago,
