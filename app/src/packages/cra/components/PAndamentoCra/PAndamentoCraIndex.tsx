...
import { PAndamentoCraTable } from "./PAndamentoCraTable";
...
export default function PAndamentoCraIndex() {
  ...
  const data = useMemo(() => titulos as PTituloInterface[], [titulos]);
  ...
  const handleGerarArquivoAndamento = () => {
    toast.info("Geração em construção", {
      description: "A geração do arquivo de andamento CRA será integrada em uma próxima etapa.",
    });
  };
      ...
      <section className="rounded-xl border bg-card p-4 shadow-xs md:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            <h1 className="text-3xl font-bold tracking-tight">CRA - Andamento/Diário</h1>
            <p className="text-sm text-muted-foreground">
              Acompanhe os títulos por status de importação e dados principais de andamento.
            </p>
          </div>
      ...
      <section className="space-y-3">
        ...
        <div className="flex justify-end">
          <Button
            type="button"
            className="bg-[#FF6B00] text-white transition-transform duration-200 hover:-translate-y-0.5 hover:scale-[1.01] hover:bg-[#E56000]"
            onClick={handleGerarArquivoAndamento}
          >
