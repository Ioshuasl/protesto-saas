...
import { PTituloProtestarBatchFilter } from './PTituloProtestarBatchFilter';
...
export default function PTituloProtestarBatchIndex() {
  ...
  const resumo = useMemo(() => {
    ...
    const pendentes = filteredData.filter((item) => (item.data_protesto ? 'R' : 'P') === 'P').length;
    const concluidos = filteredData.filter((item) => (item.data_protesto ? 'R' : 'P') !== 'P').length;
  ...
  return (
    <div className="flex w-full flex-col gap-5">
      <section className="rounded-xl border bg-card p-4 shadow-xs md:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">Concentre os títulos intimados e execute o protesto em lote com controle operacional.</p>
          </div>
          <Button
            type="button"
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm">Já protestados: <span className="font-semibold text-emerald-700">{resumo.concluidos}</span></div>
      ...
      <PTituloProtestarBatchTable
        data={paginatedData as PTituloProtestarBatchInterface[]}
        isLoading={isLoading}
